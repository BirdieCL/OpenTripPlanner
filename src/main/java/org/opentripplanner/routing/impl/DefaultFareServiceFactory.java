...
public class DefaultFareServiceFactory implements FareServiceFactory {
	...
	protected void fillFareRules(String agencyId, Collection<FareAttribute> fareAttributes, Collection<FareRule> fareRules, Map<AgencyAndId, FareRuleSet> fareRuleSet) {
		/*
		 * Create an empty FareRuleSet for each FareAttribute, as some
		 * FareAttribute may have no rules attached to them.
		 */
		for (FareAttribute fare : fareAttributes) {
			...
			if (fareRule == null) {
				...
				if (agencyId != null) {
					// TODO With the new GTFS lib, use fareAttribute.agency_id
					// directly
	...
	/**
	 * Build a specific FareServiceFactory given the config node, or fallback to
	 * the default if none specified.
	 * 
	...
	public static FareServiceFactory fromConfig(JsonNode config) {
		...
		case "seattle":
			...
			break;
		case "santiago":
			retval = new SantiagoFareServiceFactory();
			break;
		default:
